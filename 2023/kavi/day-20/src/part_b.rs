...
pub fn run() {
    ...
    //    let jj = strip("jj".to_string(), app.input.clone());
    ...
    println!(
        "Part B {}",
        find_stopper(&state, &app.input, "jj")
            * find_stopper(&state, &app.input, "gf")
            * find_stopper(&state, &app.input, "xz")
            * find_stopper(&state, &app.input, "bz")
    );
}

fn find_stopper(
    state: &HashMap<String, StateType>,
    ...
    loop {
        loop_count += 1;
        if !push_broadcast(&mut state, input, conj) {
            return loop_count as u64;
        }
    }
}
